...
| 项目章节 | 见下节。有生产实现时，**主视觉以生产代码为准**，不以课文架构图或演示脚手架为准 |
...
- [ ] 2b. 若是「项目章节」：先读仓库里的生产实现，英雄点以它为准（见下节）
...
- 候选优先**课文正文**（主题 / Learning Objectives / 双高 H2 / Key Terms / 课内代码 punchline）。**项目章节例外**：候选优先生产代码里的框架，课文只用来对名称（见「项目章节」）
...
- 课文是 Capstone，或路径在 `*-capstone-projects`，或课名含 `capstone`
- 学习者仓库里有该项目的实现（如 `codes/capstone/<项目>/`）

只有课文、没有这份代码时，仍以 `en.md` 为准，不要发明一套生产实现。

**出卡时源的优先级：**

1. **生产级代码**是主视觉。认 `prod_*.py`，或接到真实模型 / LangGraph / 框架循环的那份入口。画它实际在跑的框架：模块、数据流、停止条件、工具边界。挑重点，不画重构注释里的细枝末节。
2. **演示脚手架**（`demo_*.py`、写死的剧本、假模型）不上主视觉。只有它能说明「生产换掉了什么」时，缩成对照条，且不能比生产框图更大。
3. **课文 `en.md`** 用来对课名和目标。课文架构里这份生产代码没实现的层（沙箱厂商、传输协议、观测后端、评测榜）不画成主视觉。

产物文件名与项目对齐：`notes/cards/<phaseDir>/<lessonDir>.png`。`lessonDir` 用课文目录名（连字符），对应 `codes/capstone/` 下同名项目（下划线）。不要另起缩短的 `archi-` / `xx-` 昵称；用户明确要求旁卡时，短名仍须含项目全名。
