...
| H4 | 契约 | 15 | 中文为主；无答案/无自测/长文墙；手绘感；无 FR 堆；无 HERO/英雄点元标签 |
| H5 | 准确 | 18 | 对齐课文；无概念错；英雄点与 AI 工程钩子一致（非纯偏门数学）。项目章节改为对齐生产代码：演示脚手架或课文未实现的层占主视觉 → 扣分 |
...
6. 本课判定为模型课且架构应出现在**本张卡**上（章节卡留白路径，或 `archi-` 旁卡本身），但框图缺失、模块不可辨、或数据流不可读
7. 图上出现策划/评分元标签：`HERO`、`Hero`、`HERO 1/2`、`英雄点`、`主英雄`，或把评分维 `H1`–`H5` 当区块标题印出（读者不该看见内部术语）

不因缺 FR 徽章、未把 FR 专名做成英雄点而 Reject。不查配套 md。
章节卡走丰满路径、架构已拆到 `archi-` 时：不因本章卡无框图而 Reject。
项目章节：不因未画出课文里的未实现层而 Reject；生产代码的框架缺失或被演示脚手架盖过 → 按 H1/H5 扣分，主视觉整段是演示循环则硬门槛 1 失败。
